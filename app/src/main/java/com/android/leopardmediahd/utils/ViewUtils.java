...
public class ViewUtils {
    ...
    public static View setupActorCard(final Activity context, Picasso picasso, final com.android.leopardmediahd.functions.Actor actor) {
        View v = LayoutInflater.from(context).inflate(R.layout.horizontal_grid_item_small, null);

        // Load image
        picasso.load(actor.getUrl()).placeholder(R.color.card_background_dark).error(R.drawable.noactor).config(com.android.leopardmediahd.leopardmediahd.MizuuApplication.getBitmapConfig()).into(((ImageView) v.findViewById(R.id.cover)));

        // Set title
        ((TextView) v.findViewById(R.id.text)).setText(actor.getName());
        ((TextView) v.findViewById(R.id.text)).setTypeface(com.android.leopardmediahd.utils.TypefaceUtils.getRobotoMedium(context));
    ...
    public static void handleOnScrollChangedEvent(Activity activity, View layout, View background, String title, int height, int t, Toolbar toolbar, int toolbarColor) {
        ...
        // Update the Toolbar
        com.android.leopardmediahd.utils.ViewUtils.updateToolbarBackground(activity, toolbar, newAlpha, title, toolbarColor);
        ...
        if (!MizLib.isPortrait(activity)) {
            ...
            v.setBackgroundColor(com.android.leopardmediahd.utils.ViewUtils.adjustAlpha(Color.parseColor("#303030"), backgroundAlpha));
            v2.setBackgroundColor(com.android.leopardmediahd.utils.ViewUtils.adjustAlpha(Color.parseColor("#303030"), backgroundAlpha));
        }
    }
