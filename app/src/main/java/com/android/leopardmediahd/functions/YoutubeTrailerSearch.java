...
public class YoutubeTrailerSearch extends AsyncTask<Void, Void, String> {
	...
	protected String doInBackground(Void... params) {
		try {
			JSONObject jObject = com.android.leopardmediahd.functions.MizLib.getJSONObject(mActivity, "https://gdata.youtube.com/feeds/api/videos?q=" + mSearchQuery + "&max-results=20&alt=json&v=2");
	...
	protected void onPostExecute(String result) {
		if (result != null) {
			if (YouTubeApiServiceUtil.isYouTubeApiServiceAvailable(mActivity).equals(YouTubeInitializationResult.SUCCESS)) {
				Intent intent = YouTubeStandalonePlayer.createVideoIntent(mActivity, com.android.leopardmediahd.functions.MizLib.getYouTubeApiKey(mActivity), com.android.leopardmediahd.functions.MizLib.getYouTubeId(result), 0, false, true);
