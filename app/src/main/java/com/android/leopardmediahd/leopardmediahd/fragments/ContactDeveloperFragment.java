		...
		try {
			...
			deviceDetails.setText(
					getString(R.string.contactDeviceModel) + ": " + Build.MODEL + " (" + Build.PRODUCT + ")\n" + // Model
							...
							getResources().getString(R.string.generalInfoVersionForContact) + " " + packageInfo.versionName
					);
		} catch (Exception e) {
	...
	private void send() {
		if (!message.getText().toString().isEmpty() && message.getText().toString().length() > 5) {
			try {
				...
				String[] recipients = new String[]{"support@leopardmediahd.com"};
				emailIntent.putExtra(Intent.EXTRA_EMAIL, recipients);
				emailIntent.putExtra(Intent.EXTRA_SUBJECT, "Leopard Media HD support");
