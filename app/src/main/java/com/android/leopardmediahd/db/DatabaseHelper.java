...
import com.android.leopardmediahd.db.*;
import com.android.leopardmediahd.db.DbAdapterCollections;
import com.android.leopardmediahd.db.DbAdapterMovieMappings;
...
public class DatabaseHelper extends SQLiteOpenHelper {
	 ...
	 */
	private static final String DATABASE_CREATE_MOVIE_COLLECTIONS = "create table " + com.android.leopardmediahd.db.DbAdapterCollections.DATABASE_TABLE +
			" (" + com.android.leopardmediahd.db.DbAdapterCollections.KEY_COLLECTION_ID + " TEXT PRIMARY KEY, " + com.android.leopardmediahd.db.DbAdapterCollections.KEY_COLLECTION + " TEXT);";
	...
	private static com.android.leopardmediahd.db.DatabaseHelper mInstance;

	private DatabaseHelper(Context context) {
		super(context, DATABASE_NAME, null, DATABASE_VERSION);
	}

	public static synchronized com.android.leopardmediahd.db.DatabaseHelper getHelper(Context context) {
		if (mInstance == null)
			mInstance = new com.android.leopardmediahd.db.DatabaseHelper(context);
		return mInstance;
	}
	...
	private void upgradeMovieTable(SQLiteDatabase database) {
		...
		try {
			while (cursor.moveToNext()) {
				...
				values.put(com.android.leopardmediahd.db.DbAdapterCollections.KEY_COLLECTION, collection);
		...
		for (ContentValues values : mOldValues) {
			...
			if (!collectionsAdded.contains(values.getAsString(DbAdapterMovies.KEY_COLLECTION_ID))) {
				...
				cv.put(com.android.leopardmediahd.db.DbAdapterCollections.KEY_COLLECTION_ID, values.getAsString(DbAdapterMovies.KEY_COLLECTION_ID));
				cv.put(com.android.leopardmediahd.db.DbAdapterCollections.KEY_COLLECTION, values.getAsString(com.android.leopardmediahd.db.DbAdapterCollections.KEY_COLLECTION));
