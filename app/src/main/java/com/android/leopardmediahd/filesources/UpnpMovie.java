...
public class UpnpMovie extends com.android.leopardmediahd.abstractclasses.MovieFileSource<String> {
	...
	public UpnpMovie(Context context, com.android.leopardmediahd.functions.FileSource fileSource, boolean subFolderSearch, boolean clearLibrary, boolean disableEthernetWiFiCheck) {
		super(context, fileSource, subFolderSearch, clearLibrary, disableEthernetWiFiCheck);
	}

	@Override
	public void removeUnidentifiedFiles() {
		List<com.android.leopardmediahd.functions.DbMovie> dbMovies = getDbMovies();

		int count = dbMovies.size();
		for (int i = 0; i < count; i++) {
			if (dbMovies.get(i).isUpnpFile() && dbMovies.get(i).isUnidentified() && com.android.leopardmediahd.functions.MizLib.exists(dbMovies.get(i).getFilepath()))
                com.android.leopardmediahd.utils.MovieDatabaseUtils.deleteMovie(mContext, dbMovies.get(i).getTmdbId());
		}
	}
	...
	public void removeUnavailableFiles() {
		...
		int count = dbMovies.size();
		for (int i = 0; i < count; i++) {
			if (dbMovies.get(i).isUpnpFile() && !com.android.leopardmediahd.functions.MizLib.exists(dbMovies.get(i).getFilepath())) {
                com.android.leopardmediahd.utils.MovieDatabaseUtils.deleteMovie(mContext, dbMovies.get(i).getTmdbId());
			}
		}
	}
	...
	public List<String> searchFolder() {
		com.android.leopardmediahd.db.DbAdapterMovieMappings dbHelper = com.android.leopardmediahd.leopardmediahd.MizuuApplication.getMovieMappingAdapter();
		...
		try {
			while (cursor.moveToNext()) {// Add all movies in cursor to ArrayList of all existing movies
				existingMovies.put(cursor.getString(cache.getColumnIndex(cursor, com.android.leopardmediahd.db.DbAdapterMovieMappings.KEY_FILEPATH)), "");
			}
		} catch (Exception e) {
		} finally {
	...
	public void recursiveSearch(String folder, TreeSet<String> results) {
		mContext.getApplicationContext().bindService(new Intent(mContext, com.android.leopardmediahd.service.WireUpnpService.class), serviceConnection, Context.BIND_AUTO_CREATE);
	...
	public void addToResults(String file, long size, TreeSet<String> results) {
		if (com.android.leopardmediahd.functions.MizLib.checkFileTypes(file)) {
